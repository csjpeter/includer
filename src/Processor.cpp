...
Processor::Processor(const IFileHandler &fileHandler) : fileHandler(fileHandler)
{
}
			 ...
			 const std::string &templateDir)
{
	...
	std::string	  output(input);
	std::smatch	  match;
	const std::string contentMarkar{"@content@"};
	while (std::regex_search(output, match, frameRegex))
	{
		...
		size_t	    contentPos	 = frameContent.find(contentMarkar);
		if (contentPos == std::string::npos)
		{
			throw std::runtime_error("No @content@ marker "
						 "in frame file: "
						 + frameFile);
		}

		std::string head = frameContent.substr(0, contentPos);
		std::string tail = frameContent.substr(contentPos
						       + contentMarkar.size());
